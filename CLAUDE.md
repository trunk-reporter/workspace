...
Production runs on `gerty` (SSH: `ssh root@gerty`). See `tr-engine/CLAUDE.md` for full deployment details.
...
Each subproject with a `CLAUDE.md` has its own detailed guidance — read it before working in that directory.
...
- See `tr-engine/CLAUDE.md` for full details on data model, ingest pipeline, auth modes, and deployment
...
- See `tr-dashboard/CLAUDE.md` for store architecture, routing, and radio domain model
...
- See `imbe_asr/CLAUDE.md` for full architecture, training pipeline, and data format
...
- See `qwen3-asr-server/CLAUDE.md` for config env vars and request flow
...
- See `p25_vocoder/CLAUDE.md` for training commands and architecture
